...
2. **Targets the right version** — reads the client version from `version.xml`
   (e.g. `2.3.1.2`) and installs into `mods\<version>\`.
...
& "C:\Python27\python.exe" build\build_wotmod.py        # -> dist\com.14th_ua.garageprogressbar_3.0.0.wotmod

# 2. Install Inno Setup once (provides ISCC.exe):
winget install -e --id JRSoftware.InnoSetup

# 3. Compile:
pwsh installer\build_installer.ps1                      # -> dist\GarageProgressBar-Setup-3.0.0.exe
```
